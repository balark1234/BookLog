package com.brk.booklogger.data.leaderboard
...
import com.brk.booklogger.data.streak.ReadingStreakCalculator
    ...
    fun compute(
        ...
        val entries = kids.map { kid ->
            ...
            LocalLeaderboardEntry(
                ...
                emoji = kid.emoji,
                minutesRead = completions.sumOf(CompletedBook::minutesRead),
                pagesRead = completions.fold(0) { total, completion ->
                    total + (completion.pageCount ?: 0)
                },
                booksCompleted = completions.size,
                currentStreak = streak,
                rewardCents = balanceByKid[kid.id]
                    ?: completions.sumOf(CompletedBook::totalRewardCents),
            )
        }
        return entries
            .sortedWith(
